...
const overpassLayer = new OverpassLayer({
  ...
  feature: {
    ...
    // use the 'name' tag for grouping. this could also include a type, e.g.
    // '{{ tags.highway }}|{{ tags.name }}'
    group: '{{ tags.name }}',
  ...
  groupFeature: {
    title: '<b>{{ tags.name }}</b><br>{{ meta.ids|split(";")|join(" ") }}',
    geometry: '{{ geometry|turfConvex|turfBuffer(0.05) }}',
